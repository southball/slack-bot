...
async function main() {
    ...
    for (const T of plugins) {
        const pluginConfig: Object = config.plugins[T.id] ?? defaultPluginConfig;
        if (!isBasePluginConfig(pluginConfig) || !T.validatePluginConfig(pluginConfig)) {
    ...
    const loadPlugin = fix<string, void>(loadPlugin => async (pluginName: string) => {
        ...
        for (const requiredPlugin of T.requiredPlugins) {
            console.log(`The plugin ${pluginName} requires the plugin ${requiredPlugin}.`);
