...
export class Storage {
  ...
  async init(filename: string): Promise<void> {
    ...
    await this.database.exec("CREATE TABLE IF NOT EXISTS storage (key TEXT PRIMARY KEY, value TEXT)");
    await this.database.exec("CREATE INDEX IF NOT EXISTS storage_pk_key ON storage (key)");
  }

  async get<T>(key: string): Promise<T | undefined> {
    if (!this.database)
      throw new Error("The Storage object is not initialized.");
    const result = await this.database.get("SELECT value FROM storage WHERE key = ?", key);
